...
import { hreflangPair, URL_ALTERNATES } from './src/data/i18n.ts';
import { authorPagePairs } from './src/data/author-pairs.ts';
...
// Strony autorskie powstają z kolekcji treści, więc nie ma ich w statycznej
// mapie par językowych. Liczymy je raz, przy starcie builda.
const AUTHOR_PAIRS = authorPagePairs();
  ...
  integrations: [
    sitemap({
      ...
      serialize(item) {
        ...
        const alt = URL_ALTERNATES[path] ?? AUTHOR_PAIRS[path];
        if (alt) {
          item.links = hreflangPair(path, alt, lang).map((h) => ({
            lang: h.hreflang,
